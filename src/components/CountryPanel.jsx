  ...
  return (
    <AnimatePresence mode="wait">
      <motion.aside
        ...
        exit={{ opacity: 0, x: 24 }}
        className={`h-full overflow-y-auto border-l border-white/10 p-5 scrollbar-thin transition-all duration-300 ${
          collapsed ? 'w-[76px] px-3' : 'w-full'
        }`}
      >
        <button
          onClick={onToggleCollapse}
          className="glass mb-4 flex w-full items-center justify-between rounded-[22px] px-4 py-3 text-left shadow-2xl shadow-black/30"
        >
          <span className={`flex items-center gap-2 ${collapsed ? 'justify-center' : ''}`}>
            {collapsed ? (
              <PanelRightOpen className="h-4 w-4 text-cyan-300" />
            ) : (
              <PanelRightClose className="h-4 w-4 text-cyan-300" />
            )}
            {!collapsed && <span className="text-sm font-medium text-slate-100">Details</span>}
          </span>
        </button>

        {collapsed ? (
          <div className="glass flex flex-col items-center gap-3 rounded-[28px] py-4 shadow-2xl shadow-black/30">
            <div className="grid h-14 w-14 place-items-center rounded-2xl border border-cyan-300/20 bg-cyan-400/10 text-2xl">
              {selectedMeta.flag}
            </div>
            <div className="text-xs text-slate-400 [writing-mode:vertical-rl] rotate-180">
              {selectedMeta.name}
            </div>
          </div>
        ) : (
